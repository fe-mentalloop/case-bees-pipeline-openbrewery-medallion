...
def main():
    # Garante que /silver exista
    SILVER_DIR.mkdir(parents=True, exist_ok=True)

    # Acha todos os JSON Lines na camada Bronze
    files = glob.glob(str(BRONZE_DIR / "*.json"))
    if not files:
        print("Nenhum JSON encontrado em", BRONZE_DIR)
        return

    # Carrega tudo num DataFrame e limpa
    ...
    # Para cada state, cria a subpasta e escreve um único data.parquet
